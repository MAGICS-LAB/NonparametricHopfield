...
class MNISTBags(data_utils.Dataset):
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            # if label == 0:
            #     bag = torch.stack(random.sample(self.all_pos_img, self.pos_per_bag) + random.sample(self.all_neg_img, self.bag_size-self.pos_per_bag))
            # else:
            #     bag = torch.stack(random.sample(self.all_neg_img, self.bag_size))
