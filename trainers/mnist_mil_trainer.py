...
class Trainer:
    ...
    def _get_model(self):
        ...
        if self.config["model"] == "pooling" and self.config["mode"] != "favor":
        ...
        elif self.config["model"] == "pooling" and self.config["mode"] == "favor":
            model = MNISTModel(input_size=self.config["input_size"],
                               d_model=self.config["d_model"],
                                n_heads=self.config["n_heads"], 
                                update_steps=self.config["update_steps"], 
    ...
    def train(self):
        ...
        for epoch in range(1, self.config["epoch"]+1):
            ...
            data_log['prob'].append(self.config['prob'])
        ...
        m, p = self.config["mode"], self.config["prob"]
        # df.to_csv(f"{m}_{p}_{self.trial}.csv", index=False)
